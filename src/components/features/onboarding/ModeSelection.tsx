...
export function ModeSelection() {
    ...
    const hasSoloProfile = useUserStore((state) => state.hasSoloProfile);
    const hasFamilyProfile = useUserStore((state) => state.hasFamilyProfile);

    const handleSelectMode = (mode: 'solo' | 'family') => {
        setActiveMode(mode);

        if (mode === 'solo' && !hasSoloProfile()) {
            router.push('/onboarding/solo');
        } else if (mode === 'family' && !hasFamilyProfile()) {
            ...
            <div className="max-w-4xl w-full">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    ...
                    >
                        ...
                        <p className="text-gray-600 mb-6">
                            Suivi personnel avec objectifs individualisés, plans de repas personnalisés, et coaching IA adapté à vos besoins.
                        </p>
                        ...
                        <div className="flex items-center justify-between">
                            <span className="text-primary-600 font-semibold group-hover:translate-x-1 transition-transform inline-flex items-center gap-2">
                                {hasSoloProfile() ? 'Accéder' : 'Commencer'}
                                <ArrowRight size={20} />
                            </span>
                        </div>
                    </motion.button>
                    ...
                    >
                        ...
                        <p className="text-gray-600 mb-6">
                            Repas communs pour toute la famille, portions automatiques, liste de courses optimisée selon l'OMS.
                        </p>
                        ...
                        <div className="flex items-center justify-between">
                            <span className="text-purple-600 font-semibold group-hover:translate-x-1 transition-transform inline-flex items-center gap-2">
                                {hasFamilyProfile() ? 'Accéder' : 'Commencer'}
                                <ArrowRight size={20} />
                            </span>
                        </div>
                    </motion.button>
                ...
                >
                    <p className="text-sm text-gray-500">
                        💡 Vous pourrez créer les deux types de comptes et basculer entre eux à tout moment
                    </p>
                </motion.div>
            </div>
        </div>
